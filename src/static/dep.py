 import sqlite3 import math from sqlite3 import Error
   ...
   def create_connection(db_file):
    ...
    def QueryIns4(c, conn, data_tuple):
        ...
        if (found == 0):
            tmp0 = "select emp_name from employee where emp_id = '" + \
                data_tuple[0] + "'"
            c.execute(tmp0)
            d0 = str(c.fetchone()[0])   # dependent person name
            # print(type(d0), " + ", d0)
 
            tmp1 = "select emp_id from dependents where (dep_name = '" + \
                d0 + "' and dep_relation = '夫妻')"
            c.execute(tmp1)
            ...
            if(d1 is not None):
                ...
                if(d2 == "1" and d3 == "1"):
                    dep_update = "Update dependents set dependency = 0 where dep_id = '" + d1 + "'"
    ...
    def del_q2(c, conn, del_code):
        del_query2 = "Update country set alive = '亡國' where country_code = '" + del_code + "'"
        c.execute(del_query2)
 
    def del_q3(c, conn, del_id, del_code, del_msg):
        act = 1
        if (del_msg == "離職"):
            del_query3 = "Update stationed_agent set status = '離職' where (emp_id = '" + \
                del_id + "' and country_code = '" + del_code + "')"
        elif (del_msg == "調離原職"):
            del_query3 = "Update stationed_agent set status = '調離原職' where (emp_id = '" + \
                del_id + "' and country_code = '" + del_code + "')"
        else:
            act = 0
        if (act == 1):
    ...
    def del_q4(c, conn, del_id, del_msg):
        act = 1
        if(del_msg == "離婚"):
            del_query4 = "Update dependents set status = '離婚' where (dep_id = '" + \
                del_id + "'"
        elif(del_msg == "被人收養"):
            del_query4 = "Update dependents set status = '被人收養' where (dep_id = '" + \
                del_id + "'"
        else:
            act = 0
        if(act == 1):
    ...
    def EmployeeData(c, conn, option, info, rank):
        if(option == 1):
            ...
            for data in birthdaylist:
                msgq1 = "SELECT strftime('%s','now') - strftime('%s', '" + \
                    data + " 00:00:00')"
                c.execute(msgq1)
                x = c.fetchone()[0]
            ...
            if(info == 1):
                rank_qmsg = "select count(*) from employee where rank = '" + \
                    str(rank) + "'"
                c.execute(rank_qmsg)
    ...
    def CountryData(c, conn, option, info):
        if(option == 1):
            ...
            if(info is None):
                ...
                HasDipRelateNumberQ = "select count(distinct country_code) from country where (has_diplomatic_relation = '是' and alive = '正常' and continent_attr = '" + info + "')"
                NoDipRelateNumberQ = "select count(distinct country_code) from country where (has_diplomatic_relation = '否' and alive = '正常' and continent_attr = '" + info + "')"
            ...
            if(info is None):
                ...
                HasDipRelatePopulationQ = "select country_population from country where (has_diplomatic_relation = '是' and alive = '正常' and country_code = '" + info + "')"
                c.execute(HasDipRelatePopulationQ)
                population = c.fetchone()
 
                CountryNameQuery = "select country_name from country where country_code = '" + info + "'"
            ...
            if(info is None):
                ...
                NoDipRelatePopulationQ = "select country_population from country where (has_diplomatic_relation = '否' and alive = '正常' and country_code = '" + info + "')"
    ...
    def SagentData(c, conn, option, CountryCode):
        Semp_qmsg1 = "select count(distinct emp_id) from stationed_agent where (status = '派駐' and country_code = '" + CountryCode + "')"
        Semp_qmsg2 = "select country_area from country where country_code = '" + CountryCode + "'"
        Semp_qmsg3 = "select country_name from country where country_code = '" + CountryCode + "'"
        ...
        if(option == 1):
            ...
            c.execute(Semp_qmsg3)
            SACountryName = str(c.fetchone()[0])
            print("每平方公里", SACountryName, "員工派駐人數 = ",
                  round(EmpNumber/CountryArea))
 
    def DependtsData(c, conn, option):
 
        if(option == 1):
            AllBirthQuery = "select birthdate from dependents"
            MaleBirthQuery = "select birthdate from dependents where (dep_sex = 'M' or dep_sex = '男')"
            FemaleBirthQuery = "select birthdate from dependents where (dep_sex = 'F' or dep_sex = '女')"
    #----------------------------------------------------------------------------------------------------#
            ...
            for data in AllBirthList:
                AllAgeQuery = "SELECT strftime('%s','now') - strftime('%s', '" + \
                    data + " 00:00:00')"
    ...
    #----------------------------------------------------------------------------------------------------#
            ...
            for data in MaleBirthList:
                MaleAgeQuery = "SELECT strftime('%s','now') - strftime('%s', '" + \
                    data + " 00:00:00')"
    ...
    #----------------------------------------------------------------------------------------------------#
            ...
            for data in FemaleBirthList:
                FemaleAgeQuery = "SELECT strftime('%s','now') - strftime('%s', '" + \
                    data + " 00:00:00')"
    ...
    #----------------------------------------------------------------------------------------------------#
            ...
            MaleDepNumberQuery = "select count(*) from dependents where (dep_sex = 'M' or dep_sex = '男')"
            c.execute(MaleDepNumberQuery)
            MaleDepNumber = c.fetchone()[0]
 
            FemaleDepNumberQuery = "select count(*) from dependents where (dep_sex = 'F' or dep_sex = '女')"
    ...
    def CrossData(c, conn, option, info):
        ...
        if(option == 1):
            EmpNumber30Query = "select count(distinct emp_id) from stationed_agent where ( status = '派駐' and country_code in (select country_code from country where country_name = '" + info + "'))"
            c.execute(EmpNumber30Query)
            EmpNumber30 = c.fetchone()[0]
            print(EmpNumber30, "名員工派駐於", info)
 
        elif(option == 2):
            EmpNumberContinentQuery = "select count(distinct emp_id) from stationed_agent where ( status = '派駐' and country_code in (select country_code from country where continent_attr = '" + info + "'))"
            ...
            for data in EmpBirthList:
                AllBirthQuery = "SELECT strftime('%s','now') - strftime('%s', '" + \
                    data + " 00:00:00')"
                c.execute(AllBirthQuery)
                diff = math.floor(float(str(c.fetchone()[0])) / (60*60*24*365))
                if(diff >= 30):
                    EmpIDQuery = "select emp_id from employee where birthdate = '" + data + "'"
            ...
            if(option == 3):
                ...
                for data in DepIDList:
                    DepQuery = "select birthdate from dependents where dep_id = '" + data + "'"
                    c.execute(DepQuery)
                    DepBirthdate.append(str(c.fetchone()[0]))
 
                AvgAge = 0
                for data in DepBirthdate:
                    AgeQuery = "SELECT strftime('%s','now') - strftime('%s', '" + \
                        data + " 00:00:00')"
                    c.execute(AgeQuery)
                    age = math.floor(
                        float(str(c.fetchone()[0])) / (60*60*24*365))
                    AvgAge += age
    ...
    #-------------------------------------------------------------------------------------------#
        print("#------------------------------------#")
        print("新增員工資料中")
        data_instuple1 = ('王小明', 'A123456788', '8', 80000, '88621111111111', 'M',
                          '1971-12-15', '1995-07-01', 'Berry road 1', convertToBinaryData(
                              'C:\workspace\workspace\img1.jpg'),
                          '正常')
        data_instuple2 = ('陳小玲', 'A123456789', '8', 80000, '88621111111112', 'F',
                          '1975-08-01', '1995-08-08', 'Berry road 1', convertToBinaryData(
                              'C:\workspace\workspace\img1.jpg'),
                          '正常')
 
        data_instuple3 = ('劉小宏', 'A123456787', '10', 100000, '88621111111113', 'M',
                          '1990-12-08', '1988-10-20', 'Berry road 2', convertToBinaryData(
                              'C:\workspace\workspace\img1.jpg'),
                          '正常')
 
        data_instuple4 = ('林小妍', 'B12345678', '11', 110000, '88611000111222', 'F',
                          '1960-01-01', '1988-12-01', 'Maple road 1', convertToBinaryData(
                              'C:\workspace\workspace\img1.jpg'),
                          '正常')
 
        data_instuple5 = ('李四', 'B12345679', '11', 110000, '88612000111222', 'M',
                          '1960-12-20', '1988-11-02', 'Maple road 2', convertToBinaryData(
                              'C:\workspace\workspace\img1.jpg'),
                          '正常')
    ...
    #-------------------------------------------------------------------------------------------#
        print("#------------------------------------#")
        print("新增國家資料中")
        data_tuplec1 = ('AA4321', '澳洲', '大洋洲', 'John McGregor', 'Mary English',
                        'Jenny Wallace', 20000000, 7692024, '12345678912345', '是', '正常')
        data_tuplec2 = ('BB4321', '英國', '歐洲', 'Jake Hill', 'Justin Mann',
                        'Patrik English', 70000000, 243610, '12345678901234', '是', '正常')
        data_tuplec3 = ('CC4321', '荷蘭', '歐洲', 'Jerry Meyer', 'Joseph Bergen',
                        'Kai Jansen', 17180000, 41526, '12345678901235', '是', '正常')
        data_tuplec4 = ('DD4321', '法國', '歐洲', 'Pierre Monet', 'Mark Mann',
                        'Marco Pascal', 65000000, 643801, '12345678901236', '是', '正常')
        data_tuplec5 = ('EE4321', '德國', '歐洲', 'Johannes Littmann', 'Jannik Berkmeyer',
                        'Wilhelm Hartmann', 81000000, 357386, '12345678901237', '是', '正常')
    ...
    #-------------------------------------------------------------------------------------------#
        print("#------------------------------------#")
        print("新增派駐員工資料中")
        data_tuple_sa1 = ('A123456788', 'AA4321', '王小明',
                          '1995-12-01', '王小明', '派駐')
        data_tuple_sa2 = ('A123456789', 'BB4321', '陳小玲',
                          '1995-12-01', '陳小玲', '派駐')
        data_tuple_sa3 = ('A123456787', 'CC4321', '劉小宏',
                          '1995-12-01', '劉小宏', '派駐')
        data_tuple_sa4 = ('B12345678', 'DD4321', '林小妍',
                          '1995-12-01', '林小妍', '派駐')
        data_tuple_sa5 = ('B12345679', 'EE4321', '李四',
                          '1995-12-01', '李四', '派駐')
    ...
    #-------------------------------------------------------------------------------------------#
        print("#------------------------------------#")
        print("新增眷屬資料中")
        data_ins1 = ('A123456789', 'A123456788', '王小明',
                     'M', '夫妻', '1971-12-15', 1, '正常')
        data_ins2 = ('A123456788', 'A123456789', '陳小玲',
                     'F', '夫妻', '1975-08-01', 1, '正常')
 
        data_ins3 = ('A123456788', 'B123456780', '王大龍',
                     'M', '兒子', '2000-12-01', 0, '正常')
        data_ins4 = ('A123456789', 'B123456781', '王小珍',
                     'F', '女兒', '2003-11-15', 0, '正常')
        data_ins5 = ('A123456788', 'B123456782', '王小美',
                     'F', '女兒', '2004-11-20', 0, '正常')
