# import sqlite3
# import math from sqlite3 import Error
...
def QueryIns1(c, conn, data_tuple):
    employee_query = """ INSERT INTO employee
					('emp_name', 'emp_id', 'rank', 'salary', 'phone_number',
                       'sex', 'birthdate', 'recruit_date', 'address', 'photo', 'status') 
					VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"""
...
def QueryIns2(c, conn, data_tuple):
    country_query = """ INSERT INTO country
					('country_code', 'country_name', 'continent_attr', 'head_of_state',
                    'foreign_minister', 'liaison', 'country_population', 'country_area',
                    'contact_number', 'has_diplomatic_relation', 'alive') 
					VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"""
...
def QueryIns3(c, conn, data_tuple):
    sa_query = """ INSERT INTO stationed_agent ('emp_id', 'country_code', 'emp_name',
                'arrival_date', 'ambassador_name', 'status') VALUES (?, ?, ?, ?, ?, ?)"""
...
def QueryIns4(c, conn, data_tuple):
    ...
    if found == 0:
        ...
        if d1 is None or d2 == "1":  # None: no data
            de_query = """ INSERT INTO dependents ('emp_id', 'dep_id', 'dep_name', 'dep_sex',
                        'dep_relation', 'birthdate', 'dependency', 'status') VALUES (?, ?, ?, ?, ?, ?, ?, ?)"""
